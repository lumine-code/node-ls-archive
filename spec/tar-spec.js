...
describe("tar files", () => {
      ...
      describe("when the tree option is set to true", () => {
        describe("when the archive has no directory entries", () => {
          it("returns archive entries nested under their parent directory", (done) => {
            archive.list(
              path.join(fixturesRoot, "no-dir-entries.tgz"),
              { tree: true },
              (error, tree) => {
                ...
                expect(tree[0].children[4].children[0].getPath()).toBe(
                  path.join("package", "lib", "lister.js"),
                );
                expect(tree[0].children[4].children[1].getName()).toBe("ls-archive-cli.js");
                expect(tree[0].children[4].children[1].getPath()).toBe(
                  path.join("package", "lib", "ls-archive-cli.js"),
                );
                expect(tree[0].children[4].children[2].getName()).toBe("ls-archive.js");
                expect(tree[0].children[4].children[2].getPath()).toBe(
                  path.join("package", "lib", "ls-archive.js"),
                );
                expect(tree[0].children[4].children[3].getName()).toBe("reader.js");
                expect(tree[0].children[4].children[3].getPath()).toBe(
                  path.join("package", "lib", "reader.js"),
                );
                done();
              },
            );
  ...
  describe(".readFile()", () => {
    ...
    describe("when the path is a folder", () => {
      it("calls back with an error", (done) => {
        archive.readFile(
          path.join(fixturesRoot, "one-folder.tar"),
          `folder${path.sep}`,
          (error) => {
            expect(error.message.length).toBeGreaterThan(0);
            done();
