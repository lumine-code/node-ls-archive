...
describe("zip files", () => {
  ...
  describe(".readFile()", () => {
    ...
    describe("when the path is a folder", () => {
      it("calls back with an error", (done) => {
        archive.readFile(path.join(fixturesRoot, 'one-folder.zip'), `folder${path.sep}`, (error) => {
          expect(error.message.length).toBeGreaterThan(0);
          done();
        });
      });
    ...
    describe("when the archive contains nested directories", () => {
      it("calls back with the contents of the given path", (done) => {
        archive.readFile(path.join(fixturesRoot, 'nested.zip'), `d1${path.sep}d2${path.sep}f1.txt`, (error, contents) => {
